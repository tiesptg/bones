...
public class Query<X> {
	...
	private void constructJoin(String fromName, DbClass toClass, String alias, String joinType, List<DbField> fromFields,
			List<DbField> toFields) {
		...
		for (int j = 0; j < fromFields.size(); ++j) {
			...
			and.next(from);
			from.append(fromName).append('.').append(field.getName()).append('=').append(alias).append('.')
	...
	private Query<X> addCondition(String className, String memberName, String initialSeparator, String operator,
		...
		if (member instanceof DbRole role) {
			...
			where.append(initialSeparator).append(className).append('.').append(memberName).append(operator).append('?');
			values.add(value);
		}
		return this;
	}
