...
public class TableLayout implements LayoutManager2 {
  ...
  private Dimension layoutSize(Container parent, Sizer sizer) {
    ...
    spread(preferredWidths, moreCols, pair -> pair.preferredSize.width, pair -> pair.left,
        pair -> pair.constraints.columns);
    spread(preferredHeights, moreRows, pair -> pair.preferredSize.height, pair -> pair.top,
        pair -> pair.constraints.rows);
    return new Dimension(
        insets.left + insets.right + sum(preferredWidths) + (preferredWidths.length - 1) * gap
            + 2 * margin,
        insets.bottom + insets.top + sum(preferredHeights) + (preferredHeights.length - 1) * gap
            + 2 * margin);
  }
  ...
  private void spread(int[] size, List<Pair> more, Function<Pair, Integer> getSize,
      Function<Pair, Integer> getStart, Function<Pair, Integer> getNr) {
    for (Pair pair : more) {
      int value = 0;
